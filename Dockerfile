...
RUN apt-get update && apt-get install -y --no-install-recommends ca-certificates gcc git libpcap-dev wget && rm -rf /var/lib/apt/lists/* /var/log/* /var/cache/ldconfig/aux-cache
RUN set -eux; \
    case "${TARGETARCH}" in \
...
WORKDIR /code
COPY go/ /code/
RUN --mount=type=tmpfs,target=/root/go/ go build -trimpath -ldflags "-s -w" -o /wireshark_plugin /code/
